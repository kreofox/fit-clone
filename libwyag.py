...
argparser = argparse.ArgumentParser(description="The stupidest content tracker") #The dest="command" argument states that the name of the chosen subparser will be returned as a string in a field called command. 
argparsers = argparser.add_subparsers(title='Commands', dest='command')
argparsers.required = True
#take the parsed arguments as their unique parameter, and are responsible for processing and validating them before executing the actual comma
...
class GitRepository(object):
    ...
    def __init__(self, path, force = False): #The constructor takes an optional force which disables all checks.
        self.worktree = path
        ...
        if not force:
            vers = int(self.conf.get('core', 'repositoryformartversion'))
            if vers != 0: 
                raise Exception('Unsupported reposityformatversion %s' % vers)
#First, just a general path building function: 
...
def repo_default_config():
    ...
    ret.set('core', 'repositoryformatversion', '0') # the version of the gitdir format. 0 means the initial format, 1 the same with extensions. If > 1, git will panic; wyag will only accept 0.
    ret.set('core', 'filemod', 'false') # disable tracking of file modes (permissions) changes in the work tree.
    ret.set('core', 'bare', 'false') #indicates that this repository has a worktree. Git supports an optional worktree key which indicates the location of the worktree, if not ..; wyag doesn’t.

    return ret 
argsp = argparsers.add_parser('init', help = 'Initialize a new, empty repository.')
...
def repo_find(path='.', requierd = True):
    ...
    if parent == path:
        ...
        if requierd:
            ...
            return None
    #Recursive case
    return repo_find(parent, requierd)
...
def object_read(repo, sha): 
    '''ead object sha from Git repository repo.  Return a
...
argsp = argparsers.add_parser('cat-file', help='Provide content of repository objects')

argsp.add_argument('type',
                   metavar='type',
                   choices=['blog', 'commit', 'tag', 'tree'],
...
argsp = argparsers.add_parser('hash-object',
...
def cmd_hash_object(args):
    ...
    with open(args.path, 'rb') as fd:
        ...
        print(sha)
def object_write(fd, fmt, repo=None):
    '''Hash object, writing it to repo if provided.'''
    data = fd.read()
    #Choose constructor according to fmt argument
    match fmt:
        case b'commit'  :obj=GitCoomit(data)
...
argsp = argparsers.add_parser('log', help='Display history of a given commit')
...
class GitTree(GitObject):
    ...
    def init(self):
        self.items = list
argsp = argparsers.add_parser('ls-tree', help='Pretty-print a tree object.')
argsp.add_argument('-r',
                   dest='recursive',
                   action='store_true',
                   help = 'Recurse into sub_trees')
...
def ls_tree(repo, ref, recursive = None, prefix=''):
    ...
    for item in obj.items:
        ...
        if not (recursive and type == 'tree'): #This is leaf
...
argsp = argparsers.add_parser('checkout', help='Checkout a commit inside of directory.')
argsp.add_argument('commit',
                   help='The commit or tree to checkout.')
argsp.add_argument('path', help='The EMPTY directory to chekout on.')
...
def ref_list(repo, path=None):
    ...
    return ret
argsp = argparsers.add_parser('show-ref', help='List references.')
...
argsp = argparsers.add_parser('tag',
...
def tag_create(repo, name, ref, create_tag_object=False):
    ...
    if create_tag_object:
        ...
        # …and a tag message!
        tag.kvlmp[None] = b'A tag generated by wyag, which won`t let you customize the message!'
...
def ref_create(repo, ref_name, sha):
    with open(repo_file(repo, 'ref/'+ ref_name), 'w') as fp:
        fp.write(sha + '\n')

def object_resolve(repo, name):
    '''Resolve name to an object hasg in repo.
...
def object_find(repo, name, fmt=None, follow = True):
    ...
    while True:
        ...
        if obj.fmt == b'tag':
            ...
            return None
argsp = argparsers.add_parser('rev-parse' ,help='Parse revision (or other objects) identifiers')
...
def index_read(repo):
    ...
    for i in range(0, count):
        ...
        mode_perms = mode & ob0000000111111111
        #User ID
        uid = int.from_bytes(content[idx+28:idx+32], 'big')
        #Group ID
        gid = int.from_bytes(contex[idx+32: idx+36], 'big')
        #Size 
        fsize = int.from_bytes(contex[idx+36:idx+40], 'big')
...
argsp = argparsers.add_parser('ls-files', help='List all the stage files')
...
def cmd_ls_files(args):
    ...
    if args.verbose:
        print('Index file formate v{}, containg {} entries.'.format(index.version, len(index.entries)))
    
    for e in index.entries:
        print(e.name)
        if args.verbose:
            print(' {} with perms: {:0}'.format(
                {   0b1000: 'regular file',
                    0b1010: 'smylink',
            ...
            print(' user: {} ({}) group: {} ({})'.formate(
                pwd.getpwuid(e.uid).pw_name,
                e.uid,
                grp.getgrgid(e.gid).gr_name,
                e.gid
            ))
            print(' flag: stage={} assume_valid={}'.format(
                e.flag_stage,
                e.flag_assume_valid
            ))
argsp = argparsers.add_parser('check-ignore', help='Check path(s) against ignore rules.')
...
def gitignore_parses(raw):
    ...
    if not raw or raw[0] == '#':
        ...
        return (raw, True)
def gitignore_parses(lines):
    ret = list()

    for line in lines:
        parsed = gitignore_parses(line)
        if parsed:
            ret.append(parsed)
    return ret
...
def gitignore_read(repo):
    ...
    if os.path.exists(repo_file):
        with open(repo_file, 'r') as f:
            ret.absolute.append(gitignore_parses(f.readlines()))
    #Global configuration
    if 'XDG_CONFIG_HOME' in os.environ:
        config_home = os.environ['XDG_CONFIG_HOME']
    else:
        config_home = os.path.expanduser('~/.config')
    global_file = os.path.join(config_home, 'git/ighore')
    ...
    for entry in index.entries:
        if entry.name == '.gitignore' or entry.name.endswith('/.gitignore'):
            ...
            ret.scoped[dir_name] = gitignore_parses(lines)
    return ret
...
def check_ignore(rules, path):
    if os.path.isabs(path):
        raise Exception('This function requres path to be relative to the repository`s root')
    result = check_ignore_scoped(rules.scoped, path)
    if result != None: 
        return result
    return check_ignore_absolute(rules.absolute, path)
argsp = argparsers.add_parser('status', help='Show the working tree status.')
def cmd_status(_):
    repo = repo_find()
    index = index_read(repo)
...
def branch_get_active(repo):
    with open(repo_file(repo, 'HEAD'), 'r') as f:
        head = f.read()
    if head.startswith('rew: refs/heads/'):
        return(head[16:-1])
    else:
        return False
...
def cmd_status_index_worktree(repo, index):
    ...
    gitdir_perfix = repo.gitdir + os.path.sep
    all_files = list()
    #We begin by walking the filesystem
    for (root, _, file) in os.walk(repo.worktree, True):
        if root == repo.gitdir or root.startswith(gitdir_perfix):
            continue
        for f in files:
    ...
    with open(repo_file(repo, 'index'), 'wb') as f:
        ...
        for e in index.entries:
            ...
            if  idx % 8 != 0:
                ...
                idx += pad
argsp = argparsers.add_parser('rm', help='Remove files from the working tree and the index.')
argsp.add_argument('path', nargs='+', help='Files to remove')

def cmd_rm(args):
    repo = repo_find
    rm(repo, args.path)
def rm(remo,paths, delete=True, skip_missing=False):
...
def add(repo, paths, delete=True, skip_missing=False):
    ...
    for path in paths:
        ...
        for (abspath, relpath) in clean_paths:
            ...
            ctime_s = int(stat.st_atime)
            ctime_ns = stat.st_ctime_ns % 10 ** 9 
            mtime_s = int(stat.st_mtime)
            mtime_ns = stat.st_ctime_ns % 10 ** 9 

            entry = GitIndexEntry(ctime=(ctime_s,ctime_ns), mtime=(mtime_s,mtime_ns), dev=stat.st_ino,mode_type=0b1000, mode_perms=0o644,
                                    uid=stat.st_uid, gid= stat.st_gid,fsize=stat.st_size, sha=sha, flag_assume_valid=False,flag_stage=False,name=relpath)
            index.entries,append(entry)
        # Write the index back
        index_write(repo, index)
argsp = argparsers.add_parser('commit', help='Recorord changes to the repository.')
...
def tree_from_index(repo, index):
    ...
    #his variable will store the current tree's SHA-1.  After we're
...
def commit_create(repo, tree, parent, author, timestamp, message):
    ...
    if parent:
        commimt.kvlm[b'parent'] = parent.encode('ascii')
    #Format timezone
...
def cmd_commit(args):
    ...
    commit = commit_create(repo,
                           ...
                           gitconfig_user_git(gitconfig_read()),
                           datetime.now(),
                           args.message)
